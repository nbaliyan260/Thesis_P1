...
On an RTX 5000 Ada GPU, the frozen primary study exercised 216 scenarios across SmolLM2-135M and Qwen2.5-0.5B: 192 finite perturbations and 24 no-op controls. Full replay, sparse RECUT and all-layer activation recovery each matched the clean reference exactly on every tested state/output/continuation check. Only 2 perturbed trajectories changed a greedy token within the W-step window, both at its final step. Against the conservative full-replay implementation, median paired sparse recovery speedups were 1.30x and 1.31x, with measured median normal-path overheads of 0.35% and 0.32%, respectively. In the separate all-case sensitivity study, sparse recovery ratios against the stronger checkpoint-alias full replay were 1.29x and 1.30x (above 1 means faster). Snapshot comparison localized 192/192 actually changed old-prefix cases; its broader blind spots remain unaddressed. Recovery speedup is not an end-to-end serving-throughput claim.
...
The primary run used one NVIDIA RTX 5000 Ada Generation, BF16, batch size one and eager attention. The pinned runtime was Python 3.13.9, PyTorch 2.6.0+cu124, Transformers 4.51.3 and CUDA 12.4. Deterministic algorithms were enabled, TF32 disabled, and the execution configuration retained in metadata. The then-current 58-test CPU suite passed in the allocated HPC environment before the primary run. The eight-case debug run used separate inputs and is excluded from every primary result below.
...
| Method / endpoint | Perturbed cases | No-op controls |
| --- | --- | --- |
| No recovery: all checks equal | 0/192 | 24/24 |
| Original-slot restore: all checks equal | 1/192 | 24/24 |
| Full suffix replay: all checks equal | 192/192 | 24/24 |
| Sparse RECUT: all checks equal | 192/192 | 24/24 |
| All-layer journal: all checks equal | 192/192 | 24/24 |
| No recovery: tokens only | 190/192 | Not the full-state criterion |
| Original-slot restore: tokens only | 190/192 | Not the full-state criterion |

Every valid recovery method passed 108/108 cases on SmolLM2 and 108/108 on Qwen. SmolLM2-135M: 0/96 token-changing faults; Qwen2.5-0.5B: 2/96 token-changing faults. Original-slot restoration matched the cache in 64/192 faults and final logits in 1/192; these narrower endpoints do not replace the joint recovery criterion. There were 0 perturbed cases with no actual rounded tensor change. These counts are descriptive of the fixed matrix, not estimates of service fault impact. Multiple recovery policies evaluated on the same corrupted trajectory do not multiply the sample size. Both token divergences occurred at j=W=12: Qwen primary-052 at layer 0/cut 0 and primary-063 at layer 11/sparse cut 6. Thus no primary case executed a replay step after an observed token divergence; the primary alone does not empirically validate that branch. The faulty/no-repair state, logits or continuation were non-finite in 0/192 cases. After slot-only restoration, 126 cases kept the same W tokens but had a different cache; 63 had the same cache but different final logits. Thus not every failed repair is descendant KV corruption. The two additional uncorrected continuation decisions differed in 5/192 cases; the two within-window divergences are not the count of all observed downstream token effects. These are numerical-state findings, not measured harmful-content outcomes.
...
A separately declared, post-hoc branch test reused Qwen primary-063's exact prefix, seed and recorded fault, extending W from 12 to 14 without searching alternative faults. Its first 12 clean/faulty decisions and complete injected-fault record matched the primary. At output 12 the histories diverged; sparse replay used cut 6 for transitions 1-12 and layer 0 for 13-14. All three valid methods exactly recovered the extended state and continuation. Sparse/all/full decoder-layer work was 264/204/336, respectively. A separate CPU audit passed 1,194 checks including the saved 8.08 MiB state/logit/token archive and traces. This is one selected branch diagnostic, not an additional independent fault or a performance result. Archived tensors omit continuation; its equality remains live runner evidence.

A separate audit program passed 135,764 evidence-consistency checks with 0 failures in the pinned HPC CPU runtime. It checked all 216 rows, 1,944 repair timing samples, 3,240 normal-path timing samples, and 3 selected complete-state snapshot files (26.39 MiB). Each model's native-forward gate covered 142 positions. The auditor independently recomputed summary statistics and checked stored state tensors using weights-only CPU loading. Snapshots do not contain continuation tensors: those checks are recorded runner evidence, not independently rechecked snapshot tensors. The original-runtime audit reported no advisory warnings. The second audit on macOS retained an explicitly recorded cross-runtime advisory; it does not replace the original-runtime result. 
...
| Model / method | Full-replay speedup: median [IQR] | Normal overhead: median [IQR] |
| --- | --- | --- |
| SmolLM2-135M / sparse | 1.30x [1.01, 3.46] | 0.35% [-0.22, 0.95] |
| SmolLM2-135M / all-layer | 1.82x [1.01, 17.88] | 1.91% [1.32, 2.43] |
| Qwen2.5-0.5B / sparse | 1.31x [1.00, 3.46] | 0.32% [-0.41, 1.06] |
| Qwen2.5-0.5B / all-layer | 1.77x [1.01, 11.63] | 2.05% [1.34, 2.70] |

Each entry is computed across 96 perturbed cases per model using paired per-case medians, not a ratio of pooled medians. Median conservative full-replay times were SmolLM2-135M: 243.55 ms; Qwen2.5-0.5B: 206.26 ms. IQR denotes the 25th-75th percentiles, not a confidence interval. Five normal-path rotations across three modes do not completely balance every order position; the small observed overhead differences are noise-sensitive, not overhead guarantees. The original comparison combines reduced decoder work with fewer checkpoint-prefix copies; it must not attribute all speedup to the layer-work identity. The next section tests a stronger restoration baseline. The all-layer method is not hidden: it can recover faster while paying substantially more activation storage.

| Model | Shared full checkpoint | Extra journal payload: sparse / all |
| --- | --- | --- |
| SmolLM2-135M | 1,440.0-2,880.0 KiB | 13.5-40.5 KiB / 130.5-391.5 KiB |
| Qwen2.5-0.5B | 768.0-1,536.0 KiB | 21.0-63.0 KiB / 161.0-483.0 KiB |
...
SmolLM2-135M sparse: mean H=0.817 ms and S=75.498 ms; H/S = 1.08% of windows. Qwen2.5-0.5B sparse: mean H=0.689 ms and S=63.931 ms; H/S = 1.08% of windows. No observed deployment rate is available to show that either threshold would be met. These primary ratios use conservative copy-and-replay, not the stronger alias baseline from a separate run. The companion exploratory frontier analysis retains every case and reports layer/window/fault-shape/divergence strata; its nonnegative-cost sensitivity is not a statistical bound or a confirmed Pareto frontier.
...
The predeclared correctness gate passed on the supported frozen matrix. The 1.2x recovery-only median screening signal against the original baseline was met for both models. The sparse median recovery-only signal also survived checkpoint-alias full replay on both models. Neither result establishes the predeclared performance-thesis gate. Only the restricted snapshot-comparison cost was measured; broader end-to-end detection/commit economics, a production baseline, an acceptable holdback budget and a material advance over optimized prior-work adaptations remain unestablished. The late self-speculative-decoding comparison also leaves the novelty gate unmet; the general reuse-and-rollback rule is already known. The defensible status is a completed, auditable pilot worth a skeptical supervisor discussion, not a selected perfect thesis, a demonstrated high-impact contribution or an A/A*-ready submission.
...
Primary episodes SHA-256: 6c2d4a577eb57bd7... (full digest in metadata and the artifact manifest). The primary configuration begins bd9673777665c488..., and engine source begins c0e0949ccf415853.... Primary Slurm job 195057 and the separate audit/sensitivity jobs are recorded with timestamps and logs in notes/execution_log.md. The README provides immutable model-fetch, test, run, analysis and audit instructions. The first two PDFs preserve the prospective idea and plan; this document reports measured outcomes and limitations. One plan wording is resolved conservatively: a missing journal raises an error (fail closed), rather than implementing automatic fallback. Normal-path window computation was measured, but an end-user/private-commit latency SLO was not.
